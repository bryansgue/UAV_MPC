...
%% 2) ACELERATIONS VCP 
    ...
    vcp(:,k) = [0;0;0;0];
    %% DYNAMIC ESTIMATION
    [Test(:,k),chi_estimados(:,k+1)] = estimadaptive_dymanic_UAV_real(chi_estimados(:,k),vcp(:,k), vc(:,k), u_real(:,k), hd(:,k), h(:,k) ,Ka, L, ts, Ke);
    vref(:,k)= vc(:,k)+Test(:,k);
    ...
    %Tudata = receive(disturbance_Sub,0.1);  %(the second argument is a time-out in seconds).
    %Te(:,k) = Tudata.Axes;
