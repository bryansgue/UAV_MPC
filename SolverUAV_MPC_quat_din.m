function [opti, H0] = SolverUAV_MPC_quat_din(h,quat,v,hd,N,x_N,v_N,args,solver,k)
    
    s = size(x_N,2);
    args.p(1:s) = [h(:,k);quat(:,k);v(:,k)]; % Generacion del estado del sistema
    
    for i = 1:N % z
        args.p(s*i+1:s*i+s)=[hd(:,k+i);0;0;0];
    end 
